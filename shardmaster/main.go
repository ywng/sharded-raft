...
func main() {
	...
	// Initialize Shard Master
	store := ShardMaster{C: make(chan InputChannelType)}
	go serve(&store, r, &peers, id, raftPort)

	// Tell GRPC that s will be serving requests for the ShardMaster service and should use store (defined on line 23)
	// as the struct whose methods should be called in response.
	pb.RegisterShardMasterServer(s, &store)
	log.Printf("ShardMaster going to listen on port %v", clientPort)
